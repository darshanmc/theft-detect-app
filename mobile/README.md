...
React Native Android app for viewing the tracked car, receiving simulated or
configured FCM theft alerts, and following the car's movement while theft mode
is active.
...
2. Connect a tracker by entering its ID. Debug builds also offer demo presets.
   Confirm that the map and car-shaped marker appear after the first reading.
3. Expand **Developer tools** in the vehicle card and tap **Simulate theft alert**.
4. Confirm the theft notification, red **Theft alert active** card, and route trail.
5. Tap **This is a false alarm** and confirm to return to normal mode.
...
- **Vehicle card:** identifies the selected tracker, tracking mode, location
  reading age and full date/time, and last reported speed. Use **Change device**
  to connect a different tracker. The card scrolls for larger text/smaller screens.
- **Map following:** initially follows the vehicle. Panning, rotating, or zooming
  pauses following so new readings do not interrupt exploration. The circular
  recenter button resumes following without resetting zoom; its selected state
  is exposed to screen readers. The car's heading is relative to map rotation.
- **Reading freshness:** age uses the timestamp generated by the device and
  keeps increasing even when successful API requests return the same reading.
  A 5-second request cadence does not guarantee fresh GPS data. "Normal tracking"
  is a mode, not a guarantee that the vehicle is safe, parked, or online.
- **Unavailable data:** the app does not show a placeholder car/location before
  the first reading. A failed request replaces the loading indicator with an
  explicit message; subsequent failures retain the last-known location.
  Requests retry automatically. Map-loading failures are reported separately.
- **False alarms:** confirmation is required before high-frequency tracking
  stops. While confirmation is being submitted the action is disabled; a failed
  request keeps theft mode active and displays an error.
- **Setup:** release builds require tracker verification. Demo IDs and
  "connect without verification" are development-only troubleshooting shortcuts.
  Device IDs are not authentication or proof of vehicle ownership.

The interface intentionally uses a light palette and system fonts. Car artwork
is drawn with native views rather than an icon/font dependency; there are no
new map providers or UI framework dependencies.
...
alert** under **Developer tools**, then **This is a false alarm**, to test the
...
Before shipping UI changes, inspect normal tracking, theft mode, setup, and
connection failure on Android using mock data. Check 320 dp and typical widths,
200% font scaling, the setup keyboard, system-navigation insets, and TalkBack.
Confirm map attribution remains reachable, manual exploration survives new
readings, recenter resumes following, and 0/90/180/270-degree car headings remain
correct while rotating the map. Jest mocks cannot verify native map rendering.
